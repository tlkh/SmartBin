...
class YOLO(object):
    def __init__(self, backend,
        ...
        print("Output from", backend, self.feature_extractor.get_output_shape())    
        self.grid_h, self.grid_w = self.feature_extractor.get_output_shape()        
        features = self.feature_extractor.extract(input_image)            

        # make the object detection layer
        MULT = 3
        output = Conv2D(MULT * self.nb_box * (4 + 1 + self.nb_class), 
                        (1,1), strides=(1,1), 
                        padding='same', 
                        name='DetectionLayer', 
    ...
    def train(self, train_imgs,     # the list of images to train the model
        ...
        #optimizer = RMSprop(lr=0.001, rho=0.9, epsilon=None, decay=0.01)
        self.model.compile(loss=self.custom_loss, optimizer="rmsprop")
        ...
        ############################################
        # Print output after training completes
        ############################################
        
        print("Training cycle completed.")
        plt.figure(99, figsize=(12,4))
        plt.plot(history.history['loss'], "--", label="loss:"+str(learning_rate))
        plt.plot(history.history['val_loss'], label="val_loss:"+str(learning_rate))
        plt.legend()
        plt.tight_layout()
        plt.show()
        
        plt.figure(98, figsize=(12,4))
        plt.tight_layout()
        f, axarr = plt.subplots(1, 6, figsize=(12,6))
        
        for n in range(1,7):
            image_path = "test_4/" + str(n) + ".jpg"
            ...
            frame = self.draw_boxes(frame, boxes, ["can", "bottle"])
            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            
            axarr[n-1].imshow(frame)
